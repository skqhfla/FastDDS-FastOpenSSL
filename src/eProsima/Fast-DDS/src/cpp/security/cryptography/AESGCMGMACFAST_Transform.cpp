...
void AESGCMGMACFAST_Transform::set_for_precomputation(AESGCMGMACFAST_WriterCryptoHandleImpl& handle,
                            eprosima::fastrtps::rtps::security::KeySessionData& session)
{
    if(handle.data_session_buffer && handle.data_session_buffer->get_last() == 32){
        ...
        std::cout << "--- reset ctx & buffer ---" << std::endl;
    }
    else
        std::cout << "---free fail---" << std::endl;

    handle.session = session.session_id;

    auto ring_buffer = std::make_shared<CircularBuffer>(session.session_id);
    ...
    if(!jinho_EVP_EncryptInit_ex(handle.ctx, EVP_aes_256_gcm(), NULL, session.SessionKey.data(), iv.data()))
    {
        fprintf(stderr, "Failed to init AES-GCM with IV\n");
        exit(EXIT_FAILURE);
    }

    std::thread([ring_buffer, ctx = handle.ctx, sid](){
        std::cout << "----" << sid << " thread in set for precomputation----" << std::endl;
        ...
        SecurityException& /*exception*/)
{
    ...
    std::cout << "serialized_payload endpoint : " << local_writer->EndpointPluginAttributes << std::endl;
    ...
    std::cout << "encode serialized payload session id : " << session->session_id << std::endl;
    std::cout << "encode serialized payload session block counter : " << session->session_block_counter << std::endl;
    ...
    if (session->session_block_counter >= local_writer->max_blocks_per_session)
    {
        ...
        set_for_precomputation(impl_writer, *session);
    }
...
/*
    ...
    memcpy(initialization_vector_suffix.data() + 4, &suffix_value, sizeof(suffix_value));
    memset(initialization_vector_suffix.data(), 0, 4);
*/
   ...
   //RAND_bytes(initialization_vector_suffix.data(), initialization_vector_suffix_length);
    ...
    try
    {
        if (!serialize_SecureDataBody_precomputation(serializer, keyMat.transformation_kind, session->SessionKey, 
                    initialization_vector, output_buffer, impl_writer.data_session_buffer, impl_writer.ctx,
        ...
        SecurityException& /*exception*/)
{
    auto& impl_writer = AESGCMGMACFAST_WriterCryptoHandle::narrow(sending_datawriter_crypto);
    auto& local_writer = AESGCMGMACFAST_WriterCryptoHandle::narrow_base(sending_datawriter_crypto);
    std::cout << "datawriter_submessage endpoint : " << local_writer->EndpointPluginAttributes << std::endl;
...
//    bool do_encryption = is_vaild_gcm_kind(keyMat.transformation_kind);
    ...
    std::cout << "encode datawriter submessage session id : " << session->session_id << std::endl;
    std::cout << "encode datawriter submessage session block counter : " << session->session_block_counter << std::endl;
/*
        ...
        SecurityException& /*exception*/)
{
    ...
    std::cout << "encode datareader submessage session id : " << session->session_id << std::endl;
    std::cout << "encode datareader submessage session block counter : " << session->session_block_counter << std::endl;
        ...
        SecurityException& /*exception*/)
{
    ...
    std::cout << "encode rtps message session id : " << local_participant->Session.session_id << std::endl;
    std::cout << "encode rtps message session block counter : " << local_participant->Session.session_block_counter << std::endl;
...
/*
    ...
    uint32_t suffix_value = (local_participant->Session.session_block_counter - 1) * KEYSTREAM_SIZE;
        ...
        SecurityException& /*exception*/)
{
    ...
    if(impl_writer.ctx != NULL)
        std::cout << "----decode datawriter ctx not null----" << std::endl;
    else
        std::cout << "----decode datawriter ctx null----" << std::endl;
...
bool AESGCMGMACFAST_Transform::decode_datareader_submessage(
        ...
        SecurityException& /*exception*/)
{
    AESGCMGMACFAST_ReaderCryptoHandle& sending_reader = AESGCMGMACFAST_ReaderCryptoHandle::narrow(sending_datareader_crypto);
        ...
        SecurityException& exception)
{
    ...
    AESGCMGMACFAST_WriterCryptoHandle& sending_writer = AESGCMGMACFAST_WriterCryptoHandle::narrow(sending_datawriter_crypto);
        ...
        bool submessage)
{
    ...
    else
    {
        ...
        std::cout << "plain tex len : " << plain_buffer_len << std::endl;

        unsigned char * ks = buffer->get_keystream(plain_buffer_len, bcnt, ctx);
        std::cout << "----- 1 get stream done-----" << std::endl;
        int block_cnt;

        std::cout << "plain buffer" << plain_buffer <<  std::endl;
        if(!plain_buffer || plain_buffer_len == 0){
         std::cout << "***plain buffer is null***" << std::endl;
        }
        // JINHO: Return -1
        ...
        bool submessage)
{
    ...
    else
    {
        ...
        std::cout << "crypto text len : " << plain_buffer_len << std::endl;
